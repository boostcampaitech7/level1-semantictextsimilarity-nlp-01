...
class Dataloader(pl.LightningDataModule):
    ...
    def normalize(self, dataframe, cpu_workers = 6):
        
        def process_row(row):
            for column in self.text_columns:
                # 3회 이상 반복되는 문자를 2회로 줄이기
        ...
        with ThreadPoolExecutor(max_workers=cpu_workers) as executor:
            results = list(tqdm(executor.map(process_row, (row for _, row in dataframe.iterrows())),
                                desc = 'normalizing', total = len(dataframe)))
